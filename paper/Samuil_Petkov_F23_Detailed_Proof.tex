...
\]
The upper bound is reduced to distributions on at most three original support points and is then proved by an exact piecewise-rational covariance certificate.  A six-point multiset attains equality.  The proof covers zero variance, zero weights, coincident or collinear support, repeated selection, and the supremum-versus-maximum issue.  A semidefinite primal-dual formulation and an independent exact verification protocol are included.  This is a mathematical-analysis draft and does not assert peer-reviewed validation or bibliographic priority.
\end{abstract}
...
The remainder of the manuscript proves this theorem.  The problem originates in the data-selection framework described in \cite{HMSYopen,HMSYerm}.
...
\begin{proposition}[Continuity and closure]
Fix distinct points $z_1,\ldots,z_m$ and a simplex face determined by an index set $I$, meaning $p_i>0$ for $i\in I$ and $p_i=0$ for $i\notin I$.  On that face, $\mu(P)$, $V(P)$, and $q(P)$ are continuous functions of $p$.  Wherever $V(P)>0$, the ratio $1+q(P)/V(P)$ is continuous.  Consequently, the supremum over real finitely supported laws equals the supremum over finite multisets.
\end{proposition}
...
On the fixed support face $I$, each legal selected mean is indexed by a vector
\[
...
Positive rational points are dense in every simplex face.  Thus every positive-variance real-weight law can be approximated, without changing its support, by rational-weight laws whose ratios converge to its ratio.  Each rational-weight law is a finite multiset by the preceding proposition.  Zero-variance laws have ratio one and do not enlarge the supremum.
\end{proof}
...
\label{eq:Cp}
\end{equation}
The next lemma constructs a random legal grid point $S$ whose second moment around $p$ is exactly $C_p/5$.
...
Assign nonzero coefficients only to the four grid points below:
\begin{equation}
\begin{array}{c|c}
\toprule
s&\lambda_s\\
...
\label{eq:p1-large}
\end{equation}
Assign nonzero coefficients only to
\begin{equation}
\begin{array}{c|c}
\toprule
s&\lambda_s\\
...
\label{eq:def-Y}
\end{equation}
Every realization of $S$ represents exactly three selections because $S=k/3$ with $\sum_i k_i=3$.  The boundary part of the certificate ensures that only positive-weight support labels are used.
...
\section{Equality and extremal information}

The lower-bound example is collinear.  Equality is not confined to a line.

\subsection{Two-point equality laws}

Let the two distinct support points be $a,b$, let $d=b-a\ne0$, and put mass $\theta$ at $b$ and $1-\theta$ at $a$.  Then
\[
\mu=a+\theta d,
\qquad
V=\theta(1-\theta)\norm d^2.
...
\end{equation}
By symmetry, assume $0\le\theta\le1/2$.
...
The optimal quadratic form need not be reducible to rank one.  Consider
\[
...
For this $p$, Case B of the certificate gives strictly positive coefficients on $201$, $210$, and $300$.  If a normalized positive-semidefinite form is optimal, all three of these values must equal the common minimum $1/5$.  In tangent coordinates $u=(u_1,u_2,-u_1-u_2)$, write the form as
\[
...
This section is not needed for the direct upper-bound proof, but it explains the certificate as an exact dual solution.  Write
\[
...
&=\langle H,C_p\rangle.
\end{align*}
Thus every admissible form is realized by an actual planar support geometry.  Rank-one forms give collinear support and rank-two forms give genuinely planar support.
...
\end{equation}
The maximum exists: $C_p$ is positive definite on $T_I$, so the normalized positive-semidefinite feasible set is closed and bounded in a finite-dimensional space.  Define $\phi(p)=0$ at the three simplex vertices.  The support reduction and the two realization directions above give the exact global identity
\begin{equation}
...
For $A_s:=(s-p)(s-p)^{\mathsf T}$, introduce a scalar $t$.  The primal SDP is
\begin{equation}
\begin{array}{ll}
\text{maximize}&t\\[1mm]
...
\begin{array}{ll}
\text{minimize}&r\\[1mm]
\text{subject to}&\lambda_s\ge0,\quad\sum_s\lambda_s=1,\\
&\displaystyle\sum_s\lambda_s(s-p)(s-p)^{\mathsf T}
...
The certificate in \eqref{eq:certificate} is stronger than dual feasibility: it gives equality with $r=1/5$ in the ambient $3\times3$ matrix space.  Weak duality therefore already yields $\phi(p)\le1/5$.  The expectation proof in Section 7 is exactly the same calculation without SDP terminology.
...
An accompanying script, \texttt{verify\_samuil\_petkov\_f23.py}, performs exact symbolic verification of all nine entries of both covariance identities, exact boundary checks, exact verification of the collinear and noncollinear equality examples, and an additional rational-arithmetic stress test.  The script uses no optimization solver.  These computations are supplementary; the proof above is algebraically self-contained.
...
Proceedings of Machine Learning Research, vol. 291, 2025.
